...
class Oauth2ServiceTest {
    ...
    public void shouldGenerateToken() {
        ...
        assertEquals(accessToken.getToken(), refreshToken.getAccessToken());
        assertThat(userToken.getTokens(), Matchers.containsInAnyOrder(Token.of(refreshToken.getToken())));
    }
    ...
    public void shouldRefreshToken() {
        ...
        ArgumentCaptor<Iterable> refreshCaptor = ArgumentCaptor.forClass(Iterable.class);
        ...
        Mockito.when(securityService.findBy("otavio")).thenReturn(user);
        Mockito.when(template.get(RefreshToken.PREFIX + "refresh", RefreshToken.class))
                .thenReturn(Optional.of(new RefreshToken(userToken, token1.get(), "otavio")));
        ...
        final RefreshToken refreshToken = (RefreshToken) StreamSupport
                .stream(refreshCaptor.getValue().spliterator(), false)
                .filter(RefreshToken.class::isInstance)
                .findFirst().get();
